    ...
    return (
        <Drawer open={open} onOpenChange={handleOnOpenChange}>
            <DrawerTrigger onClick={handleOpen} asChild>
                <div className="bg-[#5B7553] text-white flex items-center w-full rounded-2xl px-5 py-3">
                    <div className="flex flex-col-reverse gap-2 md:flex-col w-full">
                        <div className="flex flex-col gap-2 md:justify-between md:flex-row md:items-center">
                            <h1 className="text-6xl md:text-3xl font-semibold">{filterOnGoing.length}</h1>
                            <p className="flex text-xs"><ArrowUpRight className="h-4 w-4" />{Number.isNaN(onGoingPercentage) ? 0 : onGoingPercentage}%</p>
                        </div>
                        <p className="text-xs font-semibold md:font-normal">On-going Tasks</p>
                    </div>
                </div>
            </DrawerTrigger>
            <DrawerContent>
                <DrawerHeader>
                    <DrawerTitle className="flex items-center"><RefreshCcw className="h-8 w-8 mr-2" />On-going Tasks</DrawerTitle>
                    <DrawerDescription className="max-w-[700px] text-gray-500 text-xs">On-going tasks represent active work items that are currently in progress within the project. These tasks are actively being worked on by team members and are not yet completed. Monitoring the progress of on-going tasks is crucial for ensuring timely completion and project success.</DrawerDescription>
                </DrawerHeader>
                <div className="px-2">
                    <ScrollArea>
