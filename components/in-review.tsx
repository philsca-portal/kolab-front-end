...
const InReview: React.FC<InReviewProps> = ({
    projectData,
    filteredTask,
    session,
    setColumn,
    ...
    const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (column === "PENDING" || column === "COMPLETED") {
            setActive(false);
            return;
        } else {
    ...
    const getIndicators = () => {
        return Array.from(document.querySelectorAll(`[data-column="IN_REVIEW"]`));
    }
    ...
    const handleDragEnd = async (e: React.DragEvent<HTMLDivElement>) => {
        ...
        const droppedTaskFromOnGoing = e.dataTransfer.getData('ONGOING');

        if (droppedTaskFromOnGoing) {
            try {
                const { id: toDoId, assignedTo: toDoEmail, teamId: teamId, title: title } = JSON.parse(droppedTaskFromOnGoing);
                if (toDoEmail !== session?.user.email) {
                    toast.error("You don't own the task.");
                } else {
                    ...
                    if (response.data.success) {
                        const moveToInReviewNotification = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/moveToInReviewNotification`, {
                            sendersId: session?.user.userId,
                            teamId: teamId,
                            title
                        });

                        if (moveToInReviewNotification.data.success) {
        ...
        const droppedTaskFromComplete = e.dataTransfer.getData('COMPLETED');

        if (droppedTaskFromComplete) {
            try {
                const { id: completeId, teamId: teamId } = JSON.parse(droppedTaskFromComplete);
    ...
    const handleDragStart = (e: React.DragEvent<HTMLDivElement>, id: string, assignedTo: string, teamId: string, title: string) => {
        setColumn("IN_REVIEW");
        e.dataTransfer.setData('INREVIEW', JSON.stringify({ id, assignedTo, teamId, title }));
    }
    ...
    const filterTask = filteredTask.filter((item) => item.status === "IN_REVIEW");
    ...
    return (
        <Card onDragEnd={handleDrag} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDragEnd} className={`${active ? 'bg-gray-500' : 'bg-card'} ${column === "PENDING" || column === "COMPLETED" ? ' opacity-20' : 'opacity-100'} max-h-[400px] overflow-hidden mx-2`}>
            <CardContent className="p-2 space-y-4">
                <div className="col-span-1">
                    <h1 className="text-sm font-extrabold uppercase">IN-REVIEW</h1>
                </div>
                <ScrollArea>
                    <div className="flex flex-col max-h-[350px] px-4">
                            ...
                            {filterTask.map((item, index) => {
                                ...
                                return (
                                    <motion.div key={index}
                                        ...
                                        <Card onDragStart={(e) => handleDragStart(e, item.id, assignedToMember?.email ?? '', assignedToMember?.TeamId ?? '', item.title)} draggable={true} className={`${theme === 'dark' ? 'bg-[#1e293b]' : 'bg-[#f1f5f9]'} flex justify-center items-center cursor-grab active:cursor-grabbing`}>
                        ...
                        <DropIndicator beforeId={"-1"} column={"IN_REVIEW"} />
                    </div>
                </ScrollArea>
            </CardContent>
        </Card>
