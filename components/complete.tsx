...
import Storage from '@/public/images/storage.gif';
...
const Complete: React.FC<CompleteProps> = ({
    ...
    const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (column === "PENDING" || column === "ON_GOING") {
            setActive(false);
            return;
        } else {
    ...
    const getIndicators = () => {
        return Array.from(document.querySelectorAll(`[data-column="COMPLETED"]`));
    }
    ...
    const handleDragEnd = async (e: React.DragEvent<HTMLDivElement>) => {
        ...
        const droppedTaskFromInReview = e.dataTransfer.getData('INREVIEW');
        if (droppedTaskFromInReview) {
            try {
                const { id: inReviewId, assignedTo: assignedTo, teamId: teamId, title: title } = JSON.parse(droppedTaskFromInReview);
                try {
                    setTaskId((previousId) => [...previousId, inReviewId]);
                    const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/getAdmin`, {
                        teamId
                    });
                    ...
                    if (response.data.success === true) {
                        if (response.data.adminEmail !== session?.user.email) {
                            ...
                            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/updatedTaskStatus`, {
                                droppedTask: inReviewId,
                                status
                            });
                            if (response.data.success) {
                                const moveToDoneNotification = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/moveToDoneNotification`, {
                                    sendersId: session?.user.userId,
                                    assignedTo: assignedTo,
                                    title
                                });

                                if (moveToDoneNotification.data.success) {
                                    const addToComplete = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/addToComplete`, {
                                        inReviewId,
                                    });

                                    if (addToComplete.data.success) {
                                        toast.success('Task Updated.');
                                        setTimeout(async () => {
                                            const deleteCompletedTask = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/deleteCompletedTask`, {
                                                inReviewId,
                                            });

                                            if (deleteCompletedTask.data.success) {
                                                setTaskId(taskId.filter(id => id !== inReviewId));
                                            } else {
    ...
    const handleDragStart = (e: React.DragEvent<HTMLDivElement>, id: string, teamId: string) => {
        setColumn("COMPLETED");
        e.dataTransfer.setData('COMPLETED', JSON.stringify({ id, teamId }));
    }
    ...
    const filterTask = filteredTask.filter((item) => item.status === "COMPLETED");
    ...
    return (
        <Card onDragEnd={handleDrag} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDragEnd} className={`${active ? 'bg-gray-500' : 'bg-card'} ${column === "PENDING" || column === "ON_GOING" ? ' opacity-20' : 'opacity-100'} max-h-[400px] overflow-hidden mx-2`}>
            <CardContent className="p-2 space-y-4">
                <div className="col-span-1">
                    <h1 className="text-sm font-extrabold uppercase">Done</h1>
                </div>
                <ScrollArea>
                    <div className="flex flex-col max-h-[350px] px-4">
                            ...
                            {filterTask.map((item, index) => {
                                ...
                                return (
                                    <motion.div key={index}
                                        layout
                                        layoutId={item.id}>
                                        <DropIndicator beforeId={index || "-1"} column={"IN_REVIEW"} />
                                        <Card onDragStart={(e) => handleDragStart(e, item.id, assignedToMember?.TeamId ?? '')} draggable={true} className={`${theme === 'dark' ? 'bg-[#1e293b]' : 'bg-[#f1f5f9]'} flex justify-center items-center cursor-grab active:cursor-grabbing`}>
                                            <CardContent className="p-4 flex flex-col space-y-2 max-w-44">
                                                {session?.user.userId === projectData.userId
                                                    &&
                                                    <div className={`flex ${taskId.includes(item.id) ? 'justify-between' : 'justify-end'}`}>
                                                        {taskId.includes(item.id)
                                                            &&
                                                            <div className='flex items-center'>
                                                                <p className='text-sm text-gray-500'>Storing..</p>
                                                                <Image src={Storage} height={20} width={20} alt='' />
                                                            </div>
                                                        }
                                                        <DropdownMenu>
                        ...
                        <DropIndicator beforeId={"-1"} column={"COMPLETED"} />
                    </div>
                </ScrollArea>
            </CardContent>
        </Card>
