...
const OnGoing: React.FC<OnGoingProps> = ({
    ...
    const getIndicators = () => {
        return Array.from(document.querySelectorAll(`[data-column="ON_GOING"]`));
    }
    ...
    const handleDragEnd = async (e: React.DragEvent<HTMLDivElement>) => {
        ...
        const droppedTask = e.dataTransfer.getData('TODO');
        if (droppedTask) {
            try {
                const { id: toDoId, assignedTo: toDoEmail } = JSON.parse(droppedTask);
                ...
                try {
                    ...
                    if (response.data.success === true) {
                        if (response.data.adminEmail !== session?.user.email) {
                            ...
                            if (response.data.success) {
                                const moveToOnGoingNotification = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/moveToOnGoingNotification`, {
                                    sendersId: session?.user.userId,
                                    assignedTo: assignedTo,
                                    title
                                });

                                if (moveToOnGoingNotification.data.success) {
                                    toast.success('Task Updated.');
                                } else {
                                    toast.error('Something went wrong.(Notification)');
                                }
    ...
    const handleDragStart = (e: React.DragEvent<HTMLDivElement>, id: string, assignedTo: string, teamId: string, title: string) => {
        setColumn("ON_GOING");
        e.dataTransfer.setData('ONGOING', JSON.stringify({ id, assignedTo, teamId, title }));
    }
    ...
    const filterTask = filteredTask.filter((item) => item.status === "ON_GOING");
    ...
    return (
        <Card onDragEnd={handleDrag} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDragEnd} className={`${active ? 'bg-gray-500' : 'bg-card'} ${column === "COMPLETED" ? ' opacity-20' : 'opacity-100'} max-h-[400px] overflow-hidden mx-2`}>
            <CardContent className="p-2 space-y-4">
                <div className="col-span-1">
                    <h1 className="text-sm font-extrabold uppercase">ON-GOING</h1>
                </div>
                <ScrollArea>
                    <div className="flex flex-col max-h-[350px] px-4">
                            ...
                            {filterTask.map((item, index) => {
                                ...
                                return (
                                    <motion.div key={index}
                                        layout
                                        layoutId={item.id}>
                                        <DropIndicator beforeId={index || "-1"} column={"ON_GOING"} />
                                        <Card onDragStart={(e) => handleDragStart(e, item.id, assignedToMember?.email ?? '', assignedToMember?.TeamId ?? '', item.title)} draggable={true} key={index} className={`${theme === 'dark' ? 'bg-[#1e293b]' : 'bg-[#f1f5f9]'} flex justify-center items-center cursor-grab active:cursor-grabbing`}>
                        ...
                        <DropIndicator beforeId={"-1"} column={"ON_GOING"} />
                    </div>
                </ScrollArea>
            </CardContent>
        </Card>
